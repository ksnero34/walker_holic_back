...
def input_report_data(json_data):
    ...
    # conn.commit()
    # title
    query ="""Insert into compTBL VALUES ( (SELECT IFNULL(MAX(compID) + 1, 1) FROM compTBL test), 'test', %s, %s, "fuckimage", "fuckimage","fuckimage", "fuck_date")"""
    curs.executemany(query, ( title, content ))
    conn.commit()
    # content
