...
def input_report_data(type, title, content, image, latitude, longitude, date):
    ...
    # conn.commit()
    # title
    query ="""Insert into compTBL VALUES ( (SELECT IFNULL(MAX(compID) + 1, 1) FROM compTBL), %s, %s, %s, %s, %s,%s, %s)"""
...
def input_walk_data(destination, diff, start, end):
    ...
    query = """Insert into walkTBL VALUES ( (SELECT IFNULL(MAX(datacode) + 1, 1) FROM walkTBL), 'test', %s, %s, %s, %s)"""
